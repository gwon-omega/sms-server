...
const environmentSchema = z.object({
  ...
  JWT_SECRET: z.string().min(32, 'JWT_SECRET must be at least 32 characters')
    .refine(
      (val) => val !== 'your-jwt-secret-key-here' && val !== 'your-jwt-secret-key-here-minimum-32-characters',
...
const productionRequirements = z.object({
  JWT_SECRET: z.string().min(64, 'JWT_SECRET should be 64+ characters in production'),
  JWT_REFRESH_SECRET: z.string().min(64, 'JWT_REFRESH_SECRET should be 64+ characters in production'),
  DATA_ENCRYPTION_KEY: z.string().min(32, 'DATA_ENCRYPTION_KEY is required in production'),
  SENTRY_DSN: z.string().url('SENTRY_DSN should be configured in production'),
});
 ...
 */
export const validateEnvironment = (env: Record<string, string | undefined> = process.env): ValidationResult => {
  ...
  if (parsedEnv.NODE_ENV !== 'production') {
    if (!env.DATA_ENCRYPTION_KEY || env.DATA_ENCRYPTION_KEY.includes('development')) {
